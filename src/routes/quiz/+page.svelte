      ...
      {#each questions as q}
        {#if q === currentQues}
          ...
          <div class="flex flex-row">
            <div class="mr-4">
              <h4 class="text-2xl">
                <span class="secondary-color font-bold">Current Score:</span>
                <span class="primary-color">{score}</span>
              </h4>
            </div>
            <div class="ml-4">
              <h4 class="text-2xl">
                <span class="primary-color">{i + 1} </span><span
                  class="secondary-color"
                >
                  out of</span
                > <span class="primary-color">{questions.length}</span>
                <span class="secondary-color">questions left!</span>
              </h4>
            </div>
          </div>
      ...
      <div class="flex flex-row">
        ...
        <div class="me-2">
          <button
            class="btn btn-secondary sm:btn-sm md:btn-md lg:btn-lg"
            on:click={nextQ}
            disabled={i === questions.length - 1}>Next</button
          >
        </div>
      </div>
    {:else}
      ...
      <div class="card">
        {#if score >= 4}
          <img
            alt="drake meme"
            src="https://i.cbc.ca/1.3279853.1532811524!/fileImage/httpImage/drake.jpg"
          />
        {:else}
          <img
            alt="drake meme"
            src="https://media1.metrotimes.com/metrotimes/imager/u/original/17685622/11-drake-hotline-bling.w710.h473.jpg"
          />
        {/if}
      </div>
      <button
